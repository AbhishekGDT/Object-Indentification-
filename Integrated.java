...
public class GDT_Main extends Activity implements View.OnClickListener{
    ...
    static RelativeLayout frameHeader, frameSystemStatus,frameActive, frameOrderScan,frameSkuDetails,
            ...
            frameDemoAdmin, frameAddSku, frameAddOrder, frameStatus, frameUser;
...
//        }
        ...
        if (v.getId() == R.id.BtnLogin) {
            ...
            if (g_strUserName.length() == 0) {
                ...
                else if (g_strUserName.equals(GDT_GlobalData.VOICE_DEMOUSERNAME)){


                    if (strPassword.toString().equals(GDT_GlobalData.VOICE_DEMOPASSWORD)) {
...
//        }
        ...
        new Thread(() -> {
            try {
                File modelDir = new File(getFilesDir(), "model-en-us");
                ...
                runOnUiThread(() -> {
                    ...
                    // Now you can start using recognizer
                });
...
//            Log.e(TAG,"odometer: "+g_BluetoothProtocol.pcb_to_app_data.odometer);
    ...
    void UpdateVoiceDemoUserScreen() {
        g_OrdersClass_fromServer = null;
        g_OrdersClass_fromServer = new OrderClass();
        g_strJsonGetorders = g_strAddSkudetails;
        g_strJsonGetHoldList = GDT_test_Json_strings.STR_JSON_RES_GET_HOLD_LIST;
        ...
        startVoicePickingFromJson(g_strAddSkudetails);
...
//                g_isU32ErrorCodeAlert = false;
    ...
    private void startVoicePickingFromJson(String json) {
        try {
            JSONObject jsonObject = new JSONObject(json);
            JSONArray assignList = jsonObject.getJSONArray("assign_list");

            if (assignList.length() > 0) {
                JSONObject assign = assignList.getJSONObject(0);

                // "items" is a list with one element that's a JSON string
                String itemsString = assign.getJSONArray("items").getString(0);

                // Now parse the string into a real JSON array
                ...
                for (int i = 0; i < itemsArray.length(); i++) {
                    ...
                    String voiceMsg = "Go to location " + location + " and pick " + qty + " " + desc;

                    // Speak it
//                    speakText(voiceMsg);

                    // Show as Toast too
                    Toast.makeText(this, voiceMsg, Toast.LENGTH_LONG).show();


                    // Optionally: wait for user confirmation before next (implement if needed)
                }
            }
        } catch (Exception e) {
...
//    private void waitForUserConfirmation(Runnable onYesConfirmed) {
//        try {
...
//                    String spoken = extractTextFromHypothesis(hypothesis);
//                    if (spoken.equalsIgnoreCase("yes")) {
//                        speechService.stop();
//                        onYesConfirmed.run(); // Call your callback
//                    }
//                }
//
...
//
//
    private void copyAssetFolder(String assetFolderName, File destinationDir) throws IOException {
        AssetManager assetManager = getAssets();
        String[] files = assetManager.list(assetFolderName);

        if (files == null || files.length == 0) {
            throw new IOException("Model folder is empty or not found in assets: " + assetFolderName);
        }
        ...
        for (String filename : files) {
            String assetPath = assetFolderName + "/" + filename;
    ...
    private void initModel() {
        Log.d(TAG, "Starting model initialization manually...");
        ...
        new Thread(() -> {
            try {
                ...
                // Step 2: Load the model from internal storage
                model = new Model(modelDir.getAbsolutePath());
                ...
                runOnUiThread(() -> {
                    setErrorState("Failed to load model: " + e.getMessage());
                });
            }
        }).start();
    }
    ...
    public static List<VoicePickingActivity.PickingTask> loadCsvTasks(Context context, String filename) {
        List<VoicePickingActivity.PickingTask> tasks = new ArrayList<>();
        try {
                ...
                if (tokens.length >= 2) {
                    ...
                    if (parts.length >= 2) {
                        try {
                            ...
                            String item = parts[1];
                            tasks.add(new VoicePickingActivity.PickingTask(location, quantity, item));
